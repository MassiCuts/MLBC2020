...
def train(weights, X, y, iterations=10,desired_accuracy=1) :
...
def accuracy():
    ...
    for i in range(total):
        smpl = X[i]
        prediction = predict(weights, smpl)
        label = y[i]
        if prediction == label:
            correct_count += 1
...
#	X, y = make_blobs(n_samples=100, centers=2, cluster_std=STD, random_state=SEED)
	...
	elapsedTime = train(weights, X, y, iterations = ITERATIONS,desired_accuracy=0.95)
