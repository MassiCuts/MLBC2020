...
def loadIrisDataSet() :
	...
	for sample, label in zip(X, y) :
		if label in [0, 1] :
			XFiltered.append(sample)
			yFiltered.append(label)
...
if __name__ == "__main__" :
	...
	THRESHOLD_ITERATIONS = 10000  # Set Number of iterations
	...
	for i in range(THRESHOLD_ITERATIONS) :
		elapsedTime += train(weights, X, y)
		accuracy = test(weights, X, y) 
