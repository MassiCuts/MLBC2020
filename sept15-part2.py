...
def train(weights, X, y, iterations=1) :
...
def linelength(X) :
	...
	for [x, y] in X :
		samplesX.append(x)
		samplesY.append(y)
	minX = round(min(samplesX))
	maxX = round(max(samplesX))
...
def loadIrisDataframe() :
	iris = datasets.load_iris()
	target = iris["target"]
	return pd.DataFrame(iris["data"], columns=["sepal length in cm", "sepal width in cm", "petal length in cm", "petal width in cm"])

# Main function
if __name__ == "__main__" :

	FILE_TO_SAVE = 'test.csv'
	df = loadIrisDataframe()
